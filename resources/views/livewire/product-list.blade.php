<div class="row">
    ...
    <div class="col-lg-3 col-sm-6 d-flex flex-column align-items-center justify-content-center product-item" x-data="{selectedProduct: 0, selectedSize: 0, selectedColor: 0, errorMessage: ''}">
        <div class="product">
            <img src="{{ asset('image/product.png') }}" alt="">
        </div>
        <div class="product-list-cart">
            <div class="product-cart-amount" id="product_amount_{{ $product->id }}">RS. <span x-text="(selectedProduct == {{ $selectedProductId }} && {{ $variantPrice }} > 0) ? {{ $variantPrice }} : {{ $product->price }}"></span></div>
        ...
        <div class="product-list-title">
            <div class="product-cart-amount" id="product_size_{{ $product->id }}">
                ...
                <p id='product-cart-variant-size-{{ $product->id }}'>
                    ...
                    <span x-data="{selectedCurrentSize: 0}" id="product_{{ $product->id }}_size_{{ $size->id }}" :class="({{ $size->id }} == selectedSize) ? 'select-variant-size' : ''" class="dot cursor product-size-dot-{{ $product->id }}" @click="selectedCurrentSize = selectedSize={{ $size->id }}; selectedProduct = {{ $product->id }}; errorMessage=''">
                        {{ $size->size }}
                    </span>
                    @endforeach
                </p>
            ...
            <div class="product-add-to-cart" id="product_color_{{ $product->id }}" x-data="{faclass: 'fa fa-check'}">
                ...
                <p id='product-cart-variant-color-{{ $product->id }}' x-data="{ faCheck: '<i :class=faclass aria-hidden=true style=color:#fff></i>' }">
                    ...
                    <span class="dot-color cursor" x-html="{{ $color->id }} == selectedColor ? faCheck : '&nbsp;' " id="product_{{ $product->id }}_color_{{ $color->id }}" @click="if(selectedSize > 0) { selectedColor = {{ $color->id }}; $wire.getPrice(selectedProduct, selectedSize, selectedColor) } else { errorMessage='Select Size'; } " style="background-color: {{ $color->code }};"></span>
                    @endforeach
                </p>
            </div>
        </div>
